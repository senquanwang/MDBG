...
Transition_dtype_2 = np.dtype([('timestep', np.int32),
                             ('state', np.float32, (1,2)),  # torch.float
                             ('action', np.int64),  # torch.LongTensor
                             ('reward', np.float32),  # torch.FloatTensor
                             ('nonterminal', np.float32)])  # torch.tensor
blank_trans_2 = (0, np.zeros((1,2)), 0, 0.0, 0.0)

Transition_dtype_3 = np.dtype([('timestep', np.int32),
                             ('state', np.float32, (1,3)),  # torch.float
                             ('action', np.int64),  # torch.LongTensor
                             ('reward', np.float32),  # torch.FloatTensor
                             ('nonterminal', np.float32)])  # torch.tensor
blank_trans_3 = (0, np.zeros((1,3)), 0, 0.0, 0.0)

# Segment tree data structure where parent node values are sum/max of children node values
class SegmentTree():
  def __init__(self, size, type):
    self.index = 0  # self.data_pointer
    self.size = size  # self.capacity
    self.full = False  # Used to track actual capacity
    self.tree_start = 2**(size-1).bit_length()-1  # Put all used node leaves on last tree level
    self.sum_tree = np.zeros((self.tree_start + self.size,), dtype=np.float32)
    if type == 2:
      self.data = np.array([blank_trans_2] * size, dtype=Transition_dtype_2)  # Build structured array
    elif type == 3:
      self.data = np.array([blank_trans_3] * size, dtype=Transition_dtype_3)  # Build structured array
...
class PrioritizedReplayBuffer():
  ...
  def __init__(self, args, capacity):
    ...
    self.beta_increment_per_sampling = args.priority_weight_increase
    ...
    if args.split:
      ...
      self.type = 2
    self.transitions = SegmentTree(capacity, self.type)  # Store transitions in a wrap-around cyclic buffer within a sum tree for querying priorities

  # Adds state and action at time t, reward and terminal at time t + 1
  def push(self, state, action, reward, next_state, nonterminal):
    state = state.to(dtype=torch.float32, device=torch.device('cpu'))  # Only store last frame and discretise to save memory
    self.transitions.append((self.t, state, action, reward, nonterminal), self.transitions.max)  # Store new transition with maximum priority
    self.t = 0 if nonterminal == 0 else self.t + 1  # Start new episodes with t = 0
  ...
  def _get_transitions(self, idxs):
    ...
    if self.type == 2:
      transitions[blank_mask] = blank_trans_2
    elif self.type == 3:
      transitions[blank_mask] = blank_trans_3
    return transitions
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    while not valid:
      ...
      # else:
      #   # print('p_total:',p_total, 'np.all((self.transitions.index - idxs) % self.capacity > self.n)', np.all((self.transitions.index - idxs) % self.capacity > self.n),
      #   #       'np.all((idxs - self.transitions.index) % self.capacity >= self.history)', np.all((idxs - self.transitions.index) % self.capacity >= self.history),
      #   #       'np.all(probs != 0)', np.all(probs != 0))
      #   # print((self.transitions.index - idxs) % self.capacity)
      #   max_length = (self.transitions.index - idxs) % self.capacity
        # print(self.transitions.index, p_total, segment_length)
  ...
  def sample(self, batch_size):
    self.priority_weight = np.min([1., self.priority_weight + self.beta_increment_per_sampling])  # max = 1
  ...
  def update_priorities(self, tree_idxs, TD_errors):
    ...
    exp_priorities = np.power(clipped_errors, self.priority_exponent)
    self.transitions.update(tree_idxs, exp_priorities)

  def __len__(self):
    len = self.capacity if self.transitions.full else self.transitions.index
    return len
...
class nstepReplayMemory(object):
    ...
    def __init__(self, args, capacity):
        ...
        self.transitions = np.array([blank_trans_2] * capacity, dtype=Transition_dtype_2)
        self.full = False
        self.index = 0  # self.data_pointer
    ...
    def push(self, state, action, reward, next_state, nonterminal):
        ...
        data = (self.t, state, action, reward, nonterminal)
        self.transitions[self.index] = data  # Store data in underlying data structure
        self.index = (self.index + 1) % self.capacity  # Update index
        self.full = self.full or self.index == 0  # Save when capacity reached
        self.t = 0 if nonterminal == 0 else self.t + 1
    # Returns the transitions with blank states where appropriate
    def _get_transitions(self, idxs):
        transition_idxs = np.arange(-self.history + 1, self.n + 1) + np.expand_dims(idxs, axis=1)
        transitions = self.transitions[transition_idxs % self.capacity]
        transitions_firsts = transitions['timestep'] == 0
        blank_mask = np.zeros_like(transitions_firsts, dtype=np.bool_)
        for t in range(self.history - 2, -1, -1):  # e.g. 2 1 0
            blank_mask[:, t] = np.logical_or(blank_mask[:, t + 1], transitions_firsts[:, t + 1])  # True if future frame has timestep 0
    ...
    def _get_samples_from_segments(self, batch_size):
        ...
        while not valid:
            ...
            if np.all((self.index - idxs) % self.capacity > self.n) and np.all((idxs - self.index) % self.capacity >= self.history):
                valid = True
        # Retrieve all required transition data (from t - h to t + n)
        transitions = self._get_transitions(idxs)
        # Create un-discretised states and nth next states
        ...
        return idxs, states, actions, R, next_states, nonterminals

    def sample(self, batch_size):
        idxs, states, actions, returns, next_states, nonterminals = self._get_samples_from_segments(batch_size)  # Get batch of valid samples

        return states, actions, returns, next_states, nonterminals

    def __len__(self):
        len = self.capacity if self.full else self.index
        return len